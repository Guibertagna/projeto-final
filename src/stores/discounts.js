...
export const useDiscounts = defineStore("discounts", () => {
  ...
  async function createDiscountsStore() {
    try {
      ...
      if(response.status === 201){
        discountStore.value = [...discountStore.value, {...response.data}]
        discountDescription.value= ""
        discountPercentage.value= ""
        discountStartDate.value= ""
        discountEndDate.value= ""
        discountProduct_id.value = ""
  ...
  async function deleteDiscounts(id) {
    try {
      ...
      if (response.status === 204) {
        discountStore.value = discountStore.value.filter(
  ...
  async function editDiscounts(id) {
    try {
      const response = await editDiscountsService(
        couponInformationEdit.value,
        id
      );
