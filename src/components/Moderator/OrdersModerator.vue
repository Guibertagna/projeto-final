<template>
  ...
  <label for="statusSelect">Alterar status:</label>
  <select
    :value="order.status"
  ...
  order.statusOrder = newStatus; // CORRETO: atribuindo string diretamente

  const response = await order.editStatus(orderId);
  if (response?.status === 200) {
    const index = order.allOrders.findIndex(o => o.id === orderId);
    if (index !== -1) {
      order.allOrders[index].status = newStatus;
    }
  } else {
    console.error('Erro ao alterar status:', response);
  }
}
